#### 一、大模型部署与应用介绍

\*\**1、部署大模型必备知识*：类OPENAI接口：RESTful风格api，大模型api接口

 			大模型相关库概念

 			windows：环境搭建

 			linux：autodl，wsl2

 			多轮对话大模型：基于fastapi，WEBUI（streanlit，gradio）

 			temperature和top\_p参数常见设置

#### 二、RESTful风格 API

***1、特点：***

 	无状态性：每个请求都是独立的，服务器不存储客户的上下文信息

 	统一接口：统一接口，就是为分布式系统设计了一套“通用协议”或“标准礼仪”统一的地址（URI）来定位资源，统一的动作（HTTP方法）来表达意图，统一的语言（JSON/XML等表述）来交换数据，统一的信号（HTTP状态码和头部）来沟通元数据，

 	客户端—服务器架构：客户端与服务器职责分离，客户端负责用户界面，服务器负责存储和处理

 	资源的表述：通过不同的格式（如JSON，XML等）来表述资源状态

 	可缓存性：客户端可以缓存服务器的响应，提高性能

***2、大模型中的RESTful API使用场景***

* 模型推理：客户端通过 POST 请求将输入(如文本、图像等)发送到大模型的 RESTful 接口，模型进行推理并返回结果。这些结果通常是以JSON 格式返回的结构化数据。
* 模型训练与更新：在一些场景中，RESTfuIAPI可以用于模型的微调。客户端上传新的训练数据并通过 API触发模型的重新训练或更新。
* 资源管理与监控：大规模模型的部署和运行涉及资源的调度和管理，如 GPU 分配模型负载均衡等。RESTfuIAPI可以用来查询当前模型的状态、资源使用情况、性能指标等。管理员通过 RESTfuI API 获取模型的健康状况，检查推理的延迟、吞吐量，甚至触发故障恢复。
* 任务队列与异步处理：在处理大模型的长时间任务时，RESTfuIAPI通过异步机制管理任务队列。客户端发送请求后，服务器通过 API 返回任务 ID，客户端可以在稍后通过 API 查询任务状态或获取结果。

***3、RESTfUIAPI在大模型中的使用优点***

* 易于集成和扩展:RESTfUIAPI具有标准化的接口，适用于各种前端系统、移动应用、其他服务模块进行交互。
* 轻量级:RESTfuIAPI通常基于 HTTP，数据传输采用JSON或XML，非常轻量易于实现跨平台、跨语言的通信。
* 无状态性和可伸缩性:无状态的设计便于扩展，系统可以轻松扩容，处理高并发请求，尤其适合大规模模型的云端部署。
* 良好的缓存支持:客户端和中间层(如CDN)可以缓存静态或重复查询的响应结果，减少负载，提升响应效率。

#### 三、使用api接口调用大模型

\*\*\*1、\*\*\*一共有三种形式：命令行工具，python的request库和openai库

***2、流程***

 	大模型-----api接口------访问方式（cmd、python）

***3、在通过 API 调用大模型之前，通常需要从大模型服务提供商那里获取哪两个关键凭证信息?***

 	需要获取 API 密钥 (API Key)，用于身份认证;以及 基础 URL (Base URL)，即 API 服务的网络地址。

***4、在调用如 chat/completions 这样的聊天类大模型 API时，messages 参数扮演什么角色?它通常包含哪些内容?***

:messages 参数用于传递对话上下文历史。它是一个包含多个字典的列表，每个字典代表一条消息，并包含消息的角色(role:如 system, user, assistant)和内容(content )

***5、调用大模型 API 时，temperature 和 max tokens 这两个参数各自的主要作用是什么?***

* temperature 主要控制模型生成内容的随机性或创造性(值越低越保守，越高越随机)。

 	首先，模型为词表中的每一个词计算一个原始分数，称为 logits。这个分数代表了模型认为该词是下一个词输出的"可能性"大小。

 	在模型的原始输出值logits经过softmax函数概率分布输出之前，进行logits / temperature计算，得出三种情况：

 		temperature ---->(0,1)之间，输出会更保守，因为概率除以一个比1小的数，大的概率会变大，小的概率会变小，他们的差异会放大，模型也就会输出概率更高的值了，所以最后输出变得相对保守

 		temperature ----->  大于1，输出更随机，概率除以一个比1大的数，高概率和低概率之间的差异变小，输出变得更加随机

 		temperature  ------->等于1.保持原样输出

*  max\_tokens 主要限制模型生成响应的最大长度(以token计数)。

 	根据具体任务，输入长度和模型动态能力进行调整设置，和temperature一起设置在对话模版中

 	格式：max\_tokens = 512

max\_tokens 主要设置在：

 	OpenAI API：client.chat.completions.create() 的 max\_tokens 参数

 	vLLM：SamplingParams 的 max\_tokens 参数

 	Hugging Face：model.generate() 的 max\_new\_tokens 参数

 	LangChain：各种LLM类的 max\_tokens 参数

***6、流式输出和非流式输出的区别，以及流式输出在哪些场景下具有优势？***

流式输出是模型一边生成一边输出文本内容，非流式输出是等待模型响应完成后一次性输出，它的优势在于可以更快地展示部分结果，提供更好的实时交互体验，尤其适合对话、代码生成等需要即时反馈的场景。

#### 四、大模型相关库与概念

### （一）相关库

***1、Hugging Face Transformers***

 	Hugging Face Transformers 是一个开源的 Python 库，支持多种预训练的自然语言处理模型，如 BERT、GPT、T5 等数百个用于不同任务的预训练模型。它可以轻松实现模型加载、微调、文本生成等功能。

链接:https://github.com/huggingface/transformers

***2、魔搭社区***

链接:https://community.modelscope.cn/.

 	ModelScope旨在打造下一代开源的模型即服务共享平台，为泛AI开发者提供灵活易用、低成本的一站式模型服务产品，让模型应用更简单。

***3、OpenAI API***

 	OpenAl 提供的 API 可以让开发者访问训练好的大语言模型，如 GPT-3 和 GPT-4，通过 API 可以进行文本生成、问答、总结等多种任务。

链接:https://openai.com/

***4、Langchain***

 	LangChain 是一个框架，用于开发由大语言模型驱动的应用程序。"它可以帮助开发者构建更复杂的交互式模型应用，如对话代理、自动化工作流等

***5、SentenceTransformers***

 	SentenceTransformers 是一个基于 transformers 库的 Python 框架，专门用于句子和文本嵌入的生成。它可以通过将句子或段落编码成高维向量，实现相似性搜索、聚类、分类等任务。其核心思想是通过预训练模型(如 BERT、ROBERTa、T5 等)来生成句子的向量表示，并支持多种语言。

***6、Deepspeed***

 	DeepSpeed 是微软开源的一个深度学习优化库，专门为大规模模型的训练和推理进行优化，帮助开发者加速模型的训练过程，降低资源消耗。

### （二）模型的概念

***1、因果语言模型（causal language models）***

 	因果语言模型(causal Language Models)，也被称为自回归语言模型(autoregressive language models)或仅解码器语言模型 (decoder-only languagemodels)，是一种机器学习模型，旨在根据序列中的前导 token 预测下一个 token换句话说，它使用之前生成的 token 作为上下文，一次生成一个 token 的文本。因果“方面指的是模型在预测下一个 token 时只考虑过去的上下文(即已生成的token)，而不考虑任何未来的 token 。

 	因果语言模型被广泛用于涉及文本补全和生成的各种自然语言处理任务。它们在生成连贯且具有上下文关联性的文本方面尤其成功，这使得它们成为现代自然语言理解和生成系统的基础。



 	在深度学习中，被称为语言模型的主要有三类:

*  	序列到序列模型(sequence-to-sequence models):T5及其类似模型序列到序列模型同时使用编码器来捕获整个输入序列，以及解码器来生成输出序列。它们广泛应用于诸如机器翻译、文本摘要等任务。
* 双向模型 (bidirectional models)或仅编码器模型(encoder-only models):BERT及其类似模型

双向模型在训练期间可以访问序列中的过去和未来上下文。由于需要未来上下文，它们无法实时生成顺序输出。它们广泛用作嵌入模型，并随后用于文本分类。

* 因果语言模型(casuallanguage models)或仅解码器模型(decoder-onlymodels): GPT及其类似模型

因果语言模型以严格向前的单向方式运行，仅根据序列中的前导词汇预测每个后续词汇。这种单向性确保了模型的预测不依赖于未来上下文，使它们适合于文本补全和生成等任务。



***2、预训练(Pre-training)和基模型(Base models)***

 	基础语言模型 (base language models)是在大量文本语料库上训练的基本模型，用于预测序列中的下一个词。它们的主要目标是捕捉语言的统计模式和结构，使它们能够生成连贯且具有上下文关联性的文本。这些模型具有多功能性，可以通过微调适应各种自然语言处理任务。虽然擅长生成流畅的文本，但它们可能需要情境学习(in-context learning)或额外训练才能遵循特定指令或有效执行复杂推理任务

*  	对于 Qwen 模型，基础模型是指那些没有“-Instruct"标识符的模型，例如Qwen2.5-7B 和 Qwen2.5-72B
*  	对于某些模型，基础模型是指那些没有“-Chat”标识符的模型。

 	**要点：使用基础模型进行情境学习、下游微调等。**

***3、后训练(Post-training)和指令微调模型(Instruction-tuned models)***

 	指令微调语言模型(Instruction-tuned language models)是专门设计用于理解并以对话风格执行特定指令的模型。

 	这些模型经过微调，能准确地解释用户命令，并能以更高的准确性和一致性执行诸如摘要、翻译和问答等任务。与在大量文本语料库上训练的基础模型不同，指令调优模型会使用包含指令示例及其预期结果的数据集进行额外训练，通常涵盖多个回合。

 	这种训练方式使它们非常适合需要特定功能的应用，同时保持生成流畅且连贯文本的能力。

对于 Qwen 模型，指令调优模型是指带有“-Instruct"后缀的模型，例如Owen2.5-7B-Instruct和Qwen2.5-72B-Instruct.

\*\*要点:使用指令微调模型进行对话式的任务执行、下游微调等。\*\*



#### （三）token和tokenization

1、token代表的是模型处理和生成的基本单位，这些token可以是单词，子词和字符，具体取决于所采用的特定tokenzition方案

2、词表大小：模型识别的唯一token总数，也就是说模型可以识别多少token，词表就有多大

3、tokenization方案，按不同的标准分为四种方案：

 	基于空格的tokenzation：将文本按空格分为很多个token

 	基于字符的tokenzation：将每个字符视为一个单独的token

 	基于子词的tokenzation：会将单词进一步分解为更小的部分，例如BPE（Byte Pair Encoding），wordpiece等技术就是基于这种思想设计的

 	基于句子的tokenzation：将文本按句子来分割

 	（1）BPE（Byte Pair Encoding）

*  		基于字符的BPE：基本单位为字符，每个字符视为一个单独的token

 		初始化词汇表:词汇表最初包含所有出现过的字符。合并操作:合并的是字符对。例如，对于字符串"hello”，初始词汇表可能包括\['h','e'，'1'，'o']，合并操作可能会将"he"合并为一个新的 token。

 	适用场景:适用于处理标准的文本数据，特别是当文本数据主要由常见字符组成时。

 		基于字节的BPE:每个字节（8位）被视为一个token

#### （四）、模型处理文本的过程：

1、文本 → 格式化:使用聊天模板组织对话结构

2.格式化文本→ Tokens:拆分成模型词汇表中的单元

3.Tokens→IDs:转换为数字表示

4.IDs → 张量:转换为模型输入的数值张量5.张量→ 模型:传递给神经网络进行计算

#### 五、在windows环境下部署大模型

1、环境搭建

* 步骤1：使用conda创建虚拟环境

 		conda create --name AI\_env python=3.11.9      （创建环境）

 		conda activate AI\_env   		（激活环境）

* 步骤2：安装必要依赖，相关库：pytorch，transformers，

 		cpu版本：pip install torch

 		GPU版本：pip install torch torchvision torchaudio --index-url        https://download.pytorch.org/whl/cu121

 		pip install transformers==4.45.0

* 步骤3：在pycharm中使用刚创建的环境

2、模型下载

* 步骤1：下载Qwen模型

 		安装魔搭社区：pip install modelscope==1.18.1

* 步骤2:在py文件中下载模型，代码如下：

 		from modelscope.hub.snapshot\_download import snapshot\_download

 		llm\_model\_dir = snapshot\_download('Qwen/Qwen2.5-0.5BInstruct',cache\_dir='models')

3、模型推理，AutoModelForCausalLM和AutoTokenizer，它们是transformers库中的两个重要组件，

 	AutoModelForCausalLM：这是一个自动化的分词器类，可以根据指定的预训练模型自动选择合适的分词器。分词器的主要功能是将原始文本转换为模型可以理解的形式，比如将句子分解成单词或子词单元，并将这些单元映射到对应的数字ID(这个过程通常称为“tokenization”)。不同的模型可能需要不同类型的分词策略，而AutoTokenizer 能够根据模型名称自动加载正确的分词器配置。

 	AutoTokenizer：这个类代表了一个用于因果语言建模(即给定前文预测下一个词的任务)的自动模型。它能够根据指定的模型名称自动加载一个适合做语言生成任务的预训练模型。这类模型常用于文本生成、对话系统等场景。

 

 	这两个类使用流程：

 	首先，使用 AutoTokenizer 加载或创建一个分词器实例，准备对输入文本进行处理。

 	然后，使用 AutoModelForcausalLm 加载一个预训练的语言模型接下来，利用分词器将原始文本转换为模型输入所需的格式。

 	最后，将处理后的数据输入到模型中，执行特定的任务，如文本生成。

4、减小模型运行时占用显存的方法

*  		1、在有一张显卡的情况下，因为pytorch处理数据是默认数据精度为32位大小的内存去处理，在实例化因果语言模型的时候，可以调用一个方法，torch\_dtype="auto”，这个方法会根自身据硬件的情况，自动选择数据类型去处理，减小显存
* 两张显卡的情况下，调用pytorch中的nn.DataPArallel,数据并行方法，





 	**显存爆了的情况下，在终端查看了python的任务进程，然后使用命令结束掉，显存就释放了**

 	终端命令： ps aux | grep python    		（查看任务进程）

 			pkill  -f python				（释放任务进程）

 	代码中查看显卡状态：import torch

 					torch.cuda.empty\_cache()  # 清理缓存



 					# 检查当前显存使用

 					print(f"GPU内存使用: {torch.cuda.memory\_allocated()/1024\*\*3:.2f} GB")

 					print(f"GPU内存缓存: {torch.cuda.memory\_reserved()/1024\*\*3:.2f} GB")

#### 六、VLLM加速推理大模型

***1、vlm是一个什么样的库？它主要通过什么技术来提升大模型推理能力？***

 	vllm是开源的大模型高速推理框架，引入了全新的的注意力算法PagedAttention，优化了kv-cache，大幅提高大模型的吞吐量。

 	PagedAttention：

 	PagedAttention 的核心思想是将大语言模型的KV缓存划分为固定大小的内存页，通过页表机制建立序列与物理页的映射关系，从而像操作系统管理虚拟内存一样高效管理注意力缓存，彻底解决了内存碎片化问题，实现了内存的高效复用和吞吐量的大幅提升。

2、

 	在linux系统下用VLLM库部署的模型在推理过程中要比在windows系统下用transformers库部署的大模型响应更快，吞吐量更大，也就是每秒处理token的数量更多，这也就是为什么在实际工作中用linux系统更多一些

***3、openai兼容服务器部署的意思：***

 	OpenAI兼容服务器部署 = 使用OpenAI接口规范 + 调用本地模型文件

 	！！操作过程注意：

*  	要将VLLM部署为一个兼容openai api协议的服务器，要先运行模块  python -m vllm.entrypoints.openai.api\_server
* 启动VLLM的openai兼容服务器时，--model参数用于指定：本地模型文件的存储路径
* 当客户端通过 OpenAI APi 规范请求 LLM 服务器时，请求体中的 model 字段应该填写：启动服务器时通过 --served-model-name 参数指定的名称

***4、如何利用 vLLM 将一个本地的大语言模型部署成一个对外提供服务的 AP 服务器，并且使其接口与 OpenAlAPI兼容?***

 	python -m vllm,entrypoints.openai.api server --model本地模型路径--served-model-name 服务名称 host 0.0.0.0--port 端囗号

 	python -m vllm.entrypoints.openai.api\_server --model /root/autodl -tmp/model/Qwen/Qwen2\_\_\_5-7B-Instruct --served-model-name Qwen-7B --host 0.0.0.0 --port 8000

 	启动服务，即可将本地大语言模型部署为兼容OpenAI API的服务器，客户端只需将base\_url指向该服务地址，就能使用标准OpenAI库进行调用。

#### 七、入门FastAPI

***1、什么是FAStAPI？***

 	FastAPI是一个基于python 3.6+版本的异步WEB应用框架，使用python类型注释构建WEB API

 	其中有三个核心依赖：

 			1. Pydantic：FastAPI 使用 pydantic 处理所有数据验证、数据序列化以及基于JSON Schema的自动模型文档。

 			2. Starlette：Starlette 是一个轻量级的 ASGI 框架/工具包，非常适合用 Python 构建异步 Web服务。FastAPI就是基于 Starlette 扩展而来的，FastAPI提供的 Request 请求报文更是直接使用了 Starlette 的 Request。

 			3. Uvicorn：Uvicorn 是一个轻量级的 ASG| 服务器，基于 uv1oop 和 httptoo1s 实现，运行速度极快。我们使用 Uvicorn 来运行 FastAPI 构建的 Web 应用。

***2、什么是路由？***

 	路由是 Web 开发中的核心概念，它定义了 URL 路径 与 处理函数 之间的映射关系。

 	简单来说，就是连接要访问的地址和要处理该地址的函数的东西，



***3、本地端口与autodl端口进行SSH配置，可以查看autodl文档***

 	https://www.autodl.com/docs/ssh\_proxy/

***4、FastAPI会生成两种文档***

*  	交互式文档：地址后面加docs（通常情况下使用）

 		FastAPI自动生成的API文档（如/docs路径下的Swagger UI）的主要用途是为开发者和用户提供交互式的API接口说明和测试平台，它能够自动根据代码中的类型注解和文档字符串生成详细的接口文档，展示所有可用的端点、请求参数格式、响应数据结构以及验证规则，使前后端开发人员能够直观理解API功能、在线调试接口而无需编写额外的文档，大大提升了开发效率和协作便利性。

* 备用文档：地址后面加redoc

***5、路径参数：***

 	路由规则：静态的放在动态的前面，因为是按顺序匹配的

#### 八、基于FastAPI构建对话机器人

1、FAstAPI基本框架

&nbsp;	from fastapi import FastAPI, Body

&nbsp;	from fastapi.responses import StreamingResponse



&nbsp;	app = FastAPI()  # 创建 FastAPI 应用实例

2、OpenAI 客户端配置

&nbsp;	api\_key = ""

&nbsp;	base\_url = ""

&nbsp;	aclient = AsyncOpenAI(api\_key=api\_key, base\_url=base\_url)

3、请求参数处理

&nbsp;	async def chat(

&nbsp;   query: str = Body(..., description="用户输入"),

&nbsp;   sys\_prompt: str = Body("你是一个有用的助手。", description="系统提示词"),

&nbsp;   history: List = Body(\[], description="历史对话"),

&nbsp;   # ... 其他参数

)

* &nbsp;	Body参数：Body 是一个用于从请求体（request body）中提取数据的工具。用于声明参数应该从 HTTP 请求的正文中获取，而不是从 URL 路径或查询参数中获取

4、历史对话管理

if history\_len > 0:

&nbsp;   history = history\[-2 \* history\_len:]  # 保留指定轮数的对话

5、消息格式构建

&nbsp;	messages.clear()

&nbsp;	messages.append({"role": "system", "content": sys\_prompt})

&nbsp;	messages.extend(history)

&nbsp;	messages.append({"role": "user", "content": query})

6、流式响应实现

&nbsp;	response = await aclient.chat.completions.create(

&nbsp;  	 	model="Qwen2-5-7B-Instruct",

&nbsp;  		 messages=messages,

&nbsp;   		stream=True  # 关键：启用流式输出

)

7、异步生成器响应

async def generate\_response():

&nbsp;   async for chunk in response:

&nbsp;       chunk\_msg = chunk.choices\[0].delta.content

&nbsp;       if chunk\_msg:

&nbsp;           yield chunk\_msg



return StreamingResponse(generate\_response(), media\_type="text/plain")

8、服务器启动

if \_\_name\_\_ == "\_\_main\_\_":

&nbsp;   import uvicorn

&nbsp;   uvicorn.run(app, host="0.0.0.0", port=6006, log\_level="info")

