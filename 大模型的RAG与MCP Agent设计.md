...
***1、Function Calling回顾***：Function Calling（函数调用）是一种允许大语言模型（llm）根据用户输入识别它需要的工具并决定何时调用改工具的机制

***2、基本工作原理如下:***

 	1.用户输入查询(Input Query)。用户通过界面或命令向系统提交一个自然语言查询(Query)

 	2. LLM 准备函数调用(Prepare Function call)大语言模型(LLM)分析用户查询，并判断是否需要调用外部工具或 API 来完成任务。如果需要，LLM 会基于已知的 函数定义(Function Definition)生成一个结构化的函数调用请求(如JSON 格式)。

 	3.调用工具或 API(Invoke Tool)

系统将 LLM 生成的函数调用请求发送到对应的 工具或 API(Tools \&APIS)。例如:搜索数据库、调用天气接口、执行计算等。

 	4.工具返回结果(Response from Tool)工具执行操作后，将结果(Tool0utput)返回给系统。这个输出通常是一个结构化数据(如JSON)，包含执行结果。

 	5.将结果作为提示输入给 LLM(Prompt with Tool Output)

系统将工具返回的结果作为新的上下文信息(Prompt)重新输入给 LLM。

LLM 结合原始查询和工具返回的数据，准备生成最终响应。

 	6.生成最终响应(Generate LLM Response)LLM 基于所有信息(原始问题+工具结果)生成自然语言回复。

最终响应返回给用户，完成整个交互过程。

这个机制让 LLM 不仅能“思考”，还能“行动"-一通过调用外部工具增强其功能，实现更复杂、更准确的任务处理能力。但是不同的功能需要封装成不同的方法，且通常需要编写代码，很难在不同的平台灵活复用。

***3、MCP介绍：***

 	Model Context Protocol (MCP)是一种标准化协议，用于管理AI模型与外部工具、数据源之间的上下文信息交换。它定义了模型何获取、使用和更新工具及数据，实现多模态、复杂任务的统一调用，这使得开发者能够以一致的方式将各种数据源、工具和功能连接到AI模型（一个中间协议层）。

 	核心价值：

 		1.统一标准：提供AI模型与外部工具，数据源之间一致的信息交换格式和协议；消除平台和工具之间的差异，让集成更加顺畅；支持动态发现和描述能力，无需预先定义所有接口。

 		2、安全与可控：用户授权机制，所有访问都需要用户明确授权；细粒度权限控制，可以控制AI访问特定资源的权限；审计追踪，完整记录所有访问和操作历史。

 		3、上下文管理：多轮对话或复杂任务时，MCP能保存用户历史、工具调用结果和任务状态；模型可基于上下文做出智能判断，提高多步骤任务的执行效率和准确性。

 		4、工具调用灵活性：新工具接入简单，生态可快速扩展；支持动态能力发现和热插拔；向后兼容，支持协议版本演进

 		5、开发者友好：提供多语言SDK和统一接口，使工具注册、调用和参数描述简单易用；支持函数装饰器快速将普通函数转化为MCP工具

4、MCP技术架构：

 	MCP采用客户端-服务器架构，实现AI应用程序与外部资源之间的标准化通信

 	核心组件：MCP主机（MCP Host），集成了MCP客户端的AI应用程序（如Claude Desktop、CUrsor）

 			MCP客户端（MCP Client），在主机应用中运行，负责与MCP服务器通信

 			MCP服务器（MCP Server），暴露特定功能与数据提供工具、资源或提示，例如：工具：使LLM能够通过服务器执行操作；资源：向LLM公开服务器中的数据和内容；提示：创建可重用的提示模版和工作流。

5、MCP通信协议：

 	MCP基于JSON-RPC 2.0协议，确保结构化、可靠的通信。JSON-RPC2.0是一个轻量级的远程过程调用(RPC,Remote Procedure call)协议，它基于JSON 格式 来定义客户端与服务器之间的请求和响应数据结构。

6、传输层选项：

&nbsp;	MCP支持多种传输方式，适应不同的部署场景：

&nbsp;	1、stabdard i/o（stdio）传输：用于本地传输，适合命令行工具和本地服务，启动简单，资源占用少

&nbsp;	2、Sever-Sent Events（SSE）传输：基于HTTP的单项通信，适合WEB环境和部署，支持通过防火墙和代理

&nbsp;	3、Streamable-HTTP（可流式HTTP）传输：支持双向通信，所有请求通过/MCP端点处理，适应需要持久化会话管理的应用

7、MCP工作原理：

&nbsp;	1、连接建立：MCP客户端与服务器建立连接并完成初始化握手

&nbsp;	2、能力发现：客户端获取服务器提供的工具，资源和提示列表】

&nbsp;	3、用户交互：用户像AI模型提出请求或询问

&nbsp;	4、需求分析：AI模型分析用户需求，确定是否需要外部资源

&nbsp;	5、权限检查：如果需要访问资源或调用工具，系统检查用户权限

&nbsp;	6、用户授权：如果需要，向用户请求操作授权

&nbsp;	7、执行操作：调用相应的工具或访问资源

&nbsp;	8、结果处理：处理操作结果并将其提供给AI模型

&nbsp;	9、生成响应：AI模型基于所有信息生成最终响应

&nbsp;	10、状态更新：更新上下文状态用于后续交互
...
https://github.com/punkpeye/awesome-mcp-servers/blob/main/RE

ADME-zh.md

5\. MCP Space：http://mcpspace.cn/

#### 十八、手写MCP服务器与客户端

***（一）、uv的介绍***

&nbsp;	uv 是一个 新一代的 Python 包管理和虚拟环境工具，它的目标就是要取代传统的 pip+venv+virtualenv+ pip-tools+ poetry 这些工具，提供一个统-、极快的体验。

主要特点:

1.极快的安装速度

&nbsp;	比 pip 快几十倍，因为它用 Rust 写的，并且做了缓存优化。。冷启动也快，不需要长时间解析依赖。

2.内置虚拟环境管理

&nbsp;	类似 conda 或 venv，但更轻量。

&nbsp;	创建环境、安装依赖一步到位，不需要切换工具。

3.兼容 pip/requirements

&nbsp;	直接支持 requirements.txt和 pyproject.toml，不用改项目结构。

4.锁文件(类似 poetry/pnpm)

&nbsp;	保证团队里依赖一致，支持可重复安装。

5.全局缓存

&nbsp;	不同项目里用到的相同包只下载一次(比conda/pip 节省空间)
